...
\pagebreak
	...
	However it first needs to be defined what a drone even is. According to the Cambridge Dictionary a drone is: \textit{an aircraft that does not have a pilot but is controlled by someone on the ground} \cite{cambridgedrone}. In this work drone will refer to a quadcopter which is a drone with four motors and propellers and not a conventional \gls{UAV}, which is mostly used for military grade drones. 

	In current conflicts the meanings of \gls{UAV} and quadcopters have become more entangled than ever. The typical hobbyist drone is now used widely and efficiently to target enemy forces. In the Ukraine war drones are one of the most effective weapons to use against the Russian aggressor for example they use drones with thermite canisters attached to them to target the enemies under tree covers \cite{thermitedrones}. Since the war began the drones also became more and more autonomous, meanwhile the drone pilots are able to only lock onto the target and let the drone follow and explode near it \cite{nytimesaidrones}. This increased use in war has also had and impact in the hobby side of the project. The most used \gls{ESC} firmware BLHeli\_32 ceased its operation due to laws being passed in Norway that penalised firms that are not able to verify that their products are not used in wars \cite{blhelidead}. Which is really difficult to do regarding that both sides in the Ukraine war are buying the \gls{Fc} no matter what software is on them.  
	...
	An important part of this "Maturitätsarbeit" is open source software. Open source software is software which everybody can download and develop in their own time. The main benefit over closed software is that it is free for everyone to have and everybody can change it for themselves if they like, but it comes with the downside that if the developing team loses interest in it and it gets outdated when it needs to work with other things. Both aspects will be found later on to be useful and time exhausting.
...
%TODO mark inside the listing the change/write a little bit smoother transition inbetween the inserted points
	\begin{lstlisting}[caption={changed line}, label=changed line, language=python, style=myPython]
