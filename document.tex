...
\end{lstlisting}
	...
	The different testflights showed promising results. The drone followed the path from the coordinates.txt file, achieving the goal of this Matura paper. The different trials showed how much one or two parameters already influence the capability of the drone to fly. Another challenge was integrating the different software components, which are not transparent and sometimes outdated. The software components often conflicted during drone takeoff, with one allowing flight while another prevented it\footnote{The prevention was the one with the upper hand.}.

	
	
	\section{Discussion and Outlook}
	This matura paper also shows how important a log file is for an autonomous system, because once the system takes  control, it is impossible to know what is happening. However, one needs to know for further tuning, tweaking, and error solving. It should be noted that the built drone is only a proof of concept and requires further testing to reach full maturity. Additionally, a (infrared) camera can be added to the RaspberryPi. Due to the nature of the RaspberryPi being a fully functioning computer, unlike an Arduino, the pictures taken by the camera could be processed directly on board for tasks such as image recognition. This would be straight-forward, because it is already integrated into the in RaspberryPi.
	...
	\listoffigures	% TODO get this shit right
