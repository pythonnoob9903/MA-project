...
\end{figure}
	...
	\begin{lstlisting}
		...
		pip3 install PyYAML
	\end{lstlisting}
	What is not said in the MavProxy Documentation, is that you need a virtual environment and can not download it without one or the this error; \lstinline|error: externally-managed-environment| pops up. To get around this issue you need to create and activate a virtual environment.
	\begin{lstlisting}[language=bash]
		python3 -m venv mavproxy-env
		source mavproxy-env/bin/activate
		pip install MAVProxy
	\end{lstlisting} 
	While trying to connect to the drone over MAVProxy via \lstinline|mavproxy.py --master=/dev/ttyUSB0 --baudrate 921600 --aircraft MyCopter| a new error appeared. 
	\begin{lstlisting}
		Failed to connect to /dev/ttyUSB0 : [Errno 2] could not open port /dev/ttyUSB0: [Errno 2] No such file or directory: '/dev/ttyUSB0'
	\end{lstlisting}
	However with the \lstinline|dmesg | tail|
