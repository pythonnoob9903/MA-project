...
\pagebreak
	...
	In todays world of faster than ever advancing technology this "Maturitätsarbeit" tries to shine a light into the corner of self-built drones.
	...
	In current conflicts the meanings of \gls{UAV} and quadcopters have become more entangled than ever. The typical hobbyist drone is now used widely and efficiently to target enemy forces. In the Ukraine war drones are one of the most effective weapons to use against the Russian aggressor for example they use drones with thermite canisters attached to them to target the enemies under tree covers \cite{thermitedrones}. Since the war began the drones also became more and more autonomous, meanwhile the drone pilots are able to only lock onto the target and let the drone follow and explode near it \cite{nytimesaidrones}. 
...
%TODO mark inside the listing the change/write a little bit smoother transition inbetween the inserted points
	...
	\end{lstlisting}
	\begin{lstlisting}[caption= output from \ref{lst:listing-autopilot}]
	...
	\begin{lstlisting}[language=python, style=myPython]
^fjasdflkjasdf 		from dronekit import connect, VehicleMode
		import time
